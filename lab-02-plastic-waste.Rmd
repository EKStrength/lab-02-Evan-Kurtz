...
Answer: It's because we want the alpha of the entire plot to change not just one part. 
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()
```
The violin plots show the density of the continent that wastes a certain amount of plastic in a visually appealing way while the box plot shows stats like the mean, median, and range. 
...
```{r plastic_waste_per_cap-mismanaged-scatter}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = mismanaged_plastic_waste_per_cap)) +
  geom_point()
```   
As the plastic waste per cap gets higher so does the mismanaged plastic waste per cap slightly. 
...
```{r plastic_waste_per_cap-mismanaged-scatter-color}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = mismanaged_plastic_waste_per_cap,
                     color = continent)) +
  geom_point()
```   
Europe appears to manage plastic waste better than most other continents. As the plastic waste per cap does up the mismanagement of waste only rises slightly, at least for the most part. Africa, on the other hand, has high mismanagement of plastic waste compared to the amount of waste per cap.  
...
```{r plastic_waste_per_cap-population-point}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = total_pop,
                     color = continent)) +
  geom_point()
```
    ```{r plastic_waste_per_cap-coastal-point}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = coastal_pop,
                     color = continent)) +
...
Yes, the coastal population appears to be more highly correlated with the plastic waste per capita compared to the general population. 
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
Interpretation: From the following plot, I can see a slight correlation between coastal population proportion and plastic waste per capita. It seems that high populations along the coast leads to higher levels of plastic waste. I'm not sure why this is. Perhaps, something to do with the coastal culture has an effect on how people steward their resources. 
